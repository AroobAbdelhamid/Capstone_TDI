...
def createmap(X_ypred_avg, singlezip):
   ...
   select_year = alt.selection_single(
        ...
        bind=alt.binding_range(min=X_ypred_avg['Year'].min(), max=X_ypred_avg['Year'].max(), step=1)
    )
...
def app():
    ...
    st.sidebar.markdown("""
    It's easy! Input in a zip code (like 54601)! You can see how housing prices will be affected by climate change for that 
...
#        y=alt.Y('value:Q', axis=alt.Axis(format='$2f', title='Price Per Square Foot')),
#        color = alt.Color('key:N', title = "Location"),
#        tooltip=[alt.Tooltip('Year', title='Year', timeUnit='year'), alt.Tooltip('LatitudeUS', title='PPSF', format='$.2f')]
#    )

#    both_charts = alt.layer(summ_chart.mark_line(color='gray', size=3),
#            summ_chart.mark_circle(size=95, color='purple')
#    ).configure_axis(
#        labelFontSize=20,
#        titleFontSize=20,
...
#   select_year = alt.selection_single(
#        name='Change', fields=['Year'], init={'Year': X_ypred_avg['Year'].min() },
#        bind=alt.binding_range(min=X_ypred_avg['Year'].min(), max=2056, step=5)
#    )
#   map_plot = pd.read_csv("map_plot_grplatlonyr.csv")
#   points = alt.Chart(map_plot).mark_circle( #X_ypred_avg
#        size=30,
...
#        longitude='Longitude:Q',
#        latitude='Latitude:Q',
#        color=alt.Color('ppsf_doll_pred', scale=alt.Scale(scheme='inferno')),#, title='Price Per Square Foot ($)')),
#        tooltip=[ alt.Tooltip('air', title='Temp', format = '2f C'), alt.Tooltip('ppsf_doll_pred', title='PPSF', format='.2f')] #'Zip',
#    ).add_selection(select_year
#    ).transform_filter(select_year)
...
#    st.sidebar.markdown("""
#    It's easy! Input in a zip code! You can see predicted housing prices and also a map  On the right, you will see a map of zip codes around the area and how
#    housing prices will be affected by climate change for those zip codes! Unavailable prediction data for your zipcode will automatically default to 64093
#    """)
